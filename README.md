...
进入游戏首先来到仙山“山门”主界面。肉鸽模式、竞技模式、无尽模式暂未开放；点击“测试模式”，会随机选择对手与玩家牌组并直接开始对战。点击“卡牌一览”进入藏经阁，可按金、木、水、火、土筛选全部卡牌，翻页查看；点击卡牌会从原位置平滑放大，点击旁边空白处或按 Esc 会将卡牌缩回原位。在卡牌一览按 Esc 也可返回山门。鼠标悬停战斗手牌可放大查看效果。伤害牌须拖到敌方角色或召唤物上，指向目标时会显示预计伤害；召唤牌须拖到我方空槽位，拖拽时才会显示空槽位。其他牌拖到手牌上方即可打出。松开在无效位置会取消出牌。点击“结束回合”让敌人行动。胜负后可重开或返回山门。

战斗采用横版对峙格局：我方立绘在左、敌方立绘在右，两者面对面。我方血条和五行能量在左下角，敌方对应的 HUD 中心对称地放在右上角；两侧数值面板共用同一套排版，只有内部顺序左右镜像。状态以小圆图标显示在我方属性栏上方、敌方属性栏下方，悬停可查看规则。能量是五个圆形灵石，敌我双方都按「金木水火土」从左到右排列。背景直接显示原图，不再叠加五行圆阵或暗色遮罩；回合提示出现后会淡出。
...
- 30 种数据定义卡牌（20 张普通牌、10 张召唤牌）、3 种敌人、3 套玩家牌组。效果通过 `effects` 顺序结算，包括伤害、治疗、抽牌、获得/削减/转换能量、随机弃牌、状态及召唤。
...
- 仙侠山门主界面：仙山云海背景、书法题字、金边模式入口和流动微光；测试模式随机直接开战。藏经阁展示全部 30 张真实卡牌预制体，支持五行筛选、分页、平滑放大与收回，召唤牌保留生命徽章。
...
- 虚弱 X：造成的伤害减少 10% × X，持有者回合结束时减少 1 层。
- 脆弱 X：受到的伤害增加 10% × X，持有者回合结束时减少 1 层。
- 蓄力 X：造成的伤害增加 10% × X，持有者回合结束时减少 1 层。
- 坚韧 X：受到的伤害减少 10% × X，持有者回合结束时减少 1 层。
- 再生 X：持有者回合结束时恢复 X 点生命，再减少 1 层。
- 护盾 X：抵消 X 点元素伤害；持有者回合开始时层数减半，奇数向上取整。
- 元素封锁：不能获得对应属性能量，也不能打出该属性卡，持续时间由效果指定。

除元素封锁外，上述状态没有单独的持续回合数。蓄力与虚弱按层数抵消，坚韧与脆弱按层数抵消；只有这四种状态最多 10 层，先抵消再对剩余层数应用上限。五行、虚弱、脆弱、蓄力、坚韧的伤害百分比先相加，再统一取整；最低伤害为 0。回合结束时先递减已有状态，再结算召唤物效果，因此本次获得的状态不会立即减少。中毒和出血直接失去生命，不经过护盾；疲劳也直接失去生命。状态效果的具体说明在图标悬停提示中查看。

## 可编辑的卡牌信息库

项目根目录的 [卡牌目录.md](卡牌目录.md) 收录全部 30 张卡牌，包含五系速查索引，以及每张牌的 ID、费用、卡面描述、规则明细、召唤物生命与触发时机、所属牌组、美术方向和资源路径。
...
支持两种协作方式：用户在对话中提出卡牌改动时，开发完成后同步到卡牌目录；用户自行编辑卡牌目录并告知后，开发 agent 检查文档变更、对照实现并落实所需调整。有歧义先澄清；文档与对话冲突时以用户最新明确要求为准。

## 卡牌设计原则

> **不设计流派，只设计元素行为和交互接口。**

- **元素有性格，但不要有剧本。** 每种元素有可辨认的倾向，但不规定它只能服务某套固定打法；同一元素内部也允许出现意料之外的牌。
- **少做寄生型卡牌。** 避免大量依赖指定状态、指定层数或专属配套才能生效的牌，例如“若有 3 层中毒，伤害翻倍”。
- **多做泛用触发。** 优先考虑获得能量、打出牌、手牌数量、受到伤害等底层事件，让不同构筑自然找到组合。
- **让卡牌通过规则发生关系。** 牌与牌的关联应来自共享的资源、状态和触发事件，不以“若你有《某张牌》”之类的卡名条件锁定组合。
- **允许怪牌和冷门牌。** 单看不强、用途不明显的牌可以存在；只要它能在某些构筑中因规则交互发挥作用，就有探索价值。

卡牌描述越简洁越好，只写这张牌直接造成的效果，例如“获得 10 层护盾。”所有状态的数量统一用“层”，伤害和生命数值才用“点”。状态的触发条件、公式和层数衰减统一放在状态图标的悬停说明中，不在每张卡牌上重复解释。

## 数据与美术

卡牌、牌组和敌人在 `data/cards.json`、`data/battles.json`；召唤物模板在 `data/summons.json`，回合开始和结束效果按数据逐项执行，后续可定义其他种类的召唤物。召唤手牌使用独立的 `ui/summon_card_view.tscn`，右下角外挂徽章显示召唤物基础生命；场上使用 `ui/summon_view.tscn`，显示轻微浮动的透明立绘与固定位置的当前生命标记，悬停后在旁边显示放大的召唤牌。透明立绘位于 `assets/summons/standee/{summon_id}.webp`，对应的 4:3 手牌插画位于 `assets/cards/generated/{card_id}.webp`。其他卡牌插画未生成时自动使用 `assets/cards/elements/{element}.webp`。角色和背景也按 ID 从 `assets/` 自动读取。界面、卡框、文字与数值由 Godot 绘制。
...
本版包含独立生成的 20 张普通牌插画、10 对召唤物透明立绘与 4:3 手牌插画、角色立绘、战斗和山门背景、透明法阵纹理。五系基础直伤牌只保留 1 费、10 点伤害的版本：锋芒、藤刺、水刃、赤焰诀、碎岩；原有 2 费 20 伤、3 费 32 伤版本已从卡池和牌组移除，其余牌组副本数量保持原样。所有现有卡牌均使用 `assets/cards/generated/{id}.webp` 独立插画；旧五行通用插画仅作缺失资源时的后备。普通牌插画生成提示词见 [普通牌美术说明](docs/card_art_prompts.md)，召唤物提示词见 [召唤物美术说明](docs/summon_art_prompts.md)。运行 `python3 tools/art_pipeline.py manifest` 可生成全部 36 个资源任务；用 `python3 tools/art_pipeline.py ingest card fire_strike 图片路径` 检查 4:3 比例并按 ID 导入新插画。
...
`ui/main_menu.gd` 管理山门和藏经阁，`ui/menu_entry.gd` 绘制模式入口，`ui/menu_atmosphere.gd` 绘制边角与流动微光。三个未开放模式使用禁用按钮；测试入口在 `battle_ui.gd` 中随机选择已有敌人与牌组，不改动战斗规则。卡牌浏览复用 `CardView` / `SummonCardView`，每页两行七列，按属性、费用、ID 排序；点击放大使用独立遮罩与 Tween，收回时沿原位置返回。新增 JSON 卡牌会自动出现在浏览列表中。题字优先使用已安装的楷体，缺少时选择已安装的宋体或其他衬线字体；先查询本机字体列表，避免触发 macOS 可选字体下载。

主界面背景保存为 `assets/backgrounds/mountain_gate.webp`，由内置 imagegen 生成；实际提示词见 [主界面美术说明](docs/main_menu_art.md)。背景与按钮分开绘制，战斗背景保持原有资源。
